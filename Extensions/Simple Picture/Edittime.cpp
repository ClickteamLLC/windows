...
int WINAPI DLLExport CreateObject(mv _far *mV, fpLevObj loPtr, LPEDATA edPtr)
{
	...
	if ( IS_COMPATIBLE(mV) )
	{
		...
		LPSTR szFileTitle = (LPSTR)calloc(_MAX_PATH, 1);
...
BOOL WINAPI EditObject (mv _far *mV, fpObjInfo oiPtr, fpLevObj loPtr, LPEDATA edPtr)
{
	...
	LPSTR szFileTitle = (LPSTR)calloc(_MAX_PATH, 1);
	LPSTR szFileName =  (LPSTR)calloc(_MAX_PATH, 1);
	memset((LPVOID)&ofn, 0, sizeof(OPENFILENAME));
	ofn.lStructSize = sizeof(OPENFILENAME);
	ofn.hwndOwner = mV->mvHEditWin;
	ofn.nFilterIndex = 1;
	...
	strcpy(szFileName, edPtr->sFilename);
	...
	if ( ChoosePicture(&ofn, TRUE, pImgMgr, NULL, PICSEL_IMAGES) )
	{
		...
		if ( pFilter.Open(szFileName) == IF_OK )
		{
			...
			// Copy new filename
			strcpy(edPtr->sFilename, szFileName);
...
/*
	...
	if ( ps != NULL )		// Do the following if this surface exists
	{
		...
		ps->DrawText(edPtr->sText, strlen(edPtr->sText), rc, dwDTFlags, edPtr->textColor, hFnt,
...
void WINAPI	DLLExport CreateFromFile (LPMV mV, LPSTR fileName, LPEDATA edPtr)
{
	...
	edPtr->dwOptions = 0;
	strcpy (edPtr->sFilename, fileName);
	edPtr->nWidth = 32;
	edPtr->nHeight = 32;
...
LPVOID WINAPI DLLExport GetPropValue(LPMV mV, LPEDATA edPtr, UINT nPropID)
{
	...
	case PROPID_FILENAME:
		return new CPropDataValue((LPCSTR)edPtr->sFilename);
...
void WINAPI DLLExport SetPropValue(LPMV mV, LPEDATA edPtr, UINT nPropID, LPVOID lParam)
{
	...
	case PROPID_FILENAME:
		...
		{
			// Set filename
			strcpy (edPtr->sFilename, (LPCSTR)((CPropDataValue*)pValue)->m_pData);
