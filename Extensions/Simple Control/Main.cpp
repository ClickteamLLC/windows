...
{
	LPSTR pText = "";		// Default = empty string (note: string expressions must never return 0)
	...
	if ( rdPtr->hWnd != NULL )
	{
		...
		if ( nTextLength != 0 )
		{
			...
			pText = (LPSTR)callRunTimeFunction(rdPtr, RFUNCTION_GETSTRINGSPACE_EX, 0, nTextLength+1);
			if ( pText != NULL )
			{
...
long WINAPI DLLExport ExpGetInt(LPRDATA rdPtr,long param1)
{
	...
	if ( rdPtr->hWnd != NULL )
	{
		...
		if ( nTextLength != 0 )
		{
			LPSTR pText = (LPSTR)malloc(nTextLength+1);
			if ( pText != NULL )
			{
				GetWindowText(rdPtr->hWnd, pText, nTextLength+1);
				lValue = atol(pText);
