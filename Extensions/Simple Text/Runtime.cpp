...
*/
	...
	rdPtr->pText = (LPSTR)malloc(strlen(edPtr->sText)+1);
	strcpy(rdPtr->pText, edPtr->sText);
...
short WINAPI DLLExport DisplayRunObject(LPRDATA rdPtr)
{
	...
	if ( ps != NULL && rdPtr->pText != NULL )
	{
		...
		// Draw text
		DWORD dwDTFlags = rdPtr->dwAlignFlags | DT_VCENTER | DT_NOPREFIX | DT_SINGLELINE | DT_END_ELLIPSIS;
		ps->DrawText(rdPtr->pText, strlen(rdPtr->pText), &rc, dwDTFlags, rdPtr->textColor, rdPtr->hFont,
