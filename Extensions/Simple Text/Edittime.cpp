...
int WINAPI DLLExport CreateObject(mv _far *mV, fpLevObj loPtr, LPEDATA edPtr)
{
	...
	if ( IS_COMPATIBLE(mV) )
	{
		...
		DWORD dwNewSize = sizeof(EDITDATA) + strlen(szDefaultText);
		...
		if ( pNewPtr != NULL )
		{
			...
			strcpy(edPtr->sText, szDefaultText);
		}
	...
	case PROPID_TEXT:
		{
			...
			if ( strlen(pStr) != strlen(edPtr->sText) )
			{
